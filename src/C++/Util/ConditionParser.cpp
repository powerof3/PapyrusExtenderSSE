	...
	namespace CONDITION
	{
		...
		ConditionDataVec ParseConditions(const std::vector<RE::BSFixedString>& a_conditionList)
		{
			...
			for (auto& condition : a_conditionList) {
				...
				//conditionItemObject
				try {
					auto str = boost::trim_copy(split_condition.at(stl::to_underlying(TYPE::kConditionItemObject)));
				...
				//functionID
				try {
					auto str = boost::trim_copy(split_condition.at(stl::to_underlying(TYPE::kFunctionID)));
					if (string::is_only_digit(str)) {
				...
				//param1
				try {
					auto str = boost::trim_copy(split_condition.at(stl::to_underlying(TYPE::kParam1)));
				...
				//param2
				try {
					auto str = boost::trim_copy(split_condition.at(stl::to_underlying(TYPE::kParam2)));
					if (is_string_valid(str)) {
				...
				try {
					...
					auto str = boost::trim_copy(split_condition.at(stl::to_underlying(TYPE::kOPCode)));
					if (string::is_only_digit(str)) {
				...
				//float
				try {
					auto str = boost::trim_copy(split_condition.at(stl::to_underlying(TYPE::kFloat)));
				...
				//operator
				try {
					auto str = boost::trim_copy(split_condition.at(stl::to_underlying(TYPE::kANDOR)));
