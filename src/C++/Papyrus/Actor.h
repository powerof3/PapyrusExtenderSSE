...
namespace Papyrus::Actor
{
	...
	namespace script
	{
		inline void EquipItem(VM* a_vm, StackID a_stackID, RE::Actor* a_actor, RE::TESForm* a_form, bool a_forceEquip, bool a_silent)
		{
		...
		std::int32_t a_type)
	{
		...
		task->AddTask([a_actor, a_armor, a_srcTXST, a_tgtTXST, a_type, targetPath]() {
			...
			const auto armorAddon = a_armor->GetArmorAddon(a_actor->GetRace());
			a_actor->VisitArmorAddon(a_armor, armorAddon, [&](bool, RE::NiAVObject& a_obj) -> bool {
	...
	inline void Register(VM& a_vm)
	{
		...
		BIND(GetActorState, true);
		BIND(GetActorSoulSize, true);
		BIND(GetCriticalStage, true);
