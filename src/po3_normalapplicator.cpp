#include "po3_normalapplicator.h"
#include <cmath>
#ifdef min
#undef min
#endif
...
#undef max
#endif
#include "NetImmerseLib/half.hpp"
...
namespace RE
{
	...
	{
		BSDynamicTriShape* dynamicTriShape = niptr_cast<BSDynamicTriShape>(geometry);
		BSTriShape* triShape = niptr_cast<BSTriShape>(geometry);
		...
		bool hasNormals = (NiSkinPartition::GetVertexFlags(vertexDesc) & NiSkinPartition::NiSkinPartition::VertexFlags::VF_NORMAL) == NiSkinPartition::VertexFlags::VF_NORMAL;
		bool hasTangents = (NiSkinPartition::GetVertexFlags(vertexDesc) & NiSkinPartition::VertexFlags::VF_TANGENT) == NiSkinPartition::VertexFlags::VF_TANGENT;
		bool hasUV = (NiSkinPartition::GetVertexFlags(vertexDesc) & NiSkinPartition::VertexFlags::VF_UV) == NiSkinPartition::VertexFlags::VF_UV;
		...
		const NiSkinInstance* skinInstance = geometry->skinInstance.get();
		if (skinInstance)
		{
			const NiSkinPartition* skinPartition = skinInstance->skinPartition.get();
			if (skinPartition)
			{
				...
				vertexBlock = dynamicTriShape ? reinterpret_cast<UInt8*>(dynamicTriShape->pDynamicData) : reinterpret_cast<UInt8*>(skinPartition->partitions[0].buffData->rawVertexData);
				...
				for (UInt32 i = 0; i < numVertices; i++)
				{
					...
					if (NiSkinPartition::GetVertexFlags(vertexDesc) & NiSkinPartition::VertexFlags::VF_UV)
					{
				...
				{
					for (UInt32 t = 0; t < skinPartition->partitions[p].triangles * 3; ++t)
					{
