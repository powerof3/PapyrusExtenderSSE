...
namespace Papyrus::ObjectReference
{
		...
		RE::BGSKeyword* a_add)
	{
		...
		if (const auto form = a_ref->GetBaseObject(); form) {
			Serialization::Form::Keywords::GetSingleton()->PapyrusApply(form, a_add, Serialization::Form::kAdd);
		}
	}
		...
		RE::BGSKeyword* a_remove)
	{
		...
		return form && Serialization::Form::Keywords::GetSingleton()->PapyrusApply(form, a_remove, Serialization::Form::kRemove);
	}
		...
		RE::BSFixedString a_nodeName)
	{
		...
		for (const auto& [id, matString] : MATERIAL::materialMap) {
			if (boost::icontains(matString, a_newMaterialType.c_str())) {
				newID = id;
				break;
			}
			if (!a_oldMaterialType.empty() && boost::icontains(matString, a_oldMaterialType.c_str())) {
				oldID = id;
				break;
			}
		}
